...
const Paciente = ({paciente, setPaciente, eliminandoPacientee}) => {

    const {nombre, propietario, email,alta,sintomas, id} = paciente

    const handleEliminar = () =>{
        const respuesta = confirm('Deseas realmente eliminar el registro?')
        if(respuesta){
            eliminandoPacientee(id)
        }
    }
  ...
  return (
    <div className="mx-5 my-5 bg-white shadow-md px-5 py-10 rounded-xl " >
        ...
        <div className="flex justify-between mt-10" >
            ...
            <button
             type="button"
             className="py-2 px-10 bg-red-700 hover:bg-red-600 text-white font-bold uppercase rounded-lg "
